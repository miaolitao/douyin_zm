...
import { Layout, Button, Card, Avatar, Badge, Tag, Rate } from 'antd'
import { 
  HeartOutlined, 
  MessageOutlined, 
  ShareAltOutlined, 
...
const Food: React.FC = () => {
  ...
  const foodVideos = useMemo(() => {
    return videos.filter(video => video.category === '美食')
  }, [videos])

  const displayedVideos = useMemo(() => {
    return foodVideos.slice(0, currentPage * pageSize)
  }, [foodVideos, currentPage, pageSize])
  ...
  const handleScroll = () => {
    ...
    if (scrollTop + clientHeight >= scrollHeight - 100) {
      const hasMore = currentPage * pageSize < foodVideos.length
      if (hasMore) {
        setLoadingMore(true)
        setTimeout(() => {
  ...
  }, [loading, loadingMore, currentPage, foodVideos.length])
      ...
      <Content style={{ height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{
              ...
              <CoffeeOutlined style={{ fontSize: '20px', color: '#fff' }} />
            </div>
            <div>
              <h1 style={{ 
                color: '#fff', 
          ...
          <div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {['全部美食', '家常菜', '甜品', '小吃', '火锅', '烧烤', '西餐', '日韩料理'].map(tag => (
              <Tag 
                key={tag}
                style={{
                  backgroundColor: tag === '全部美食' ? '#ff6b35' : 'rgba(255, 255, 255, 0.1)',
                  color: tag === '全部美食' ? '#fff' : '#888',
                  border: 'none',
                  borderRadius: '16px',
                  padding: '4px 12px',
                  cursor: 'pointer'
        ...
        >
          ...
          >
                      ...
                      <div className="videoImageInner">
                        <div className="videoImageContent">
                          ...
                          <div style={{
                            ...
                            <CoffeeOutlined style={{ fontSize: '12px' }} />
                            美食
                          </div>
                        ...
                        <div className="videoStats">
                          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                            ...
                            <Button 
                              type="text" 
                              size="small"
                              icon={<StarOutlined style={{ color: '#ff6b35' }} />}
                              style={{ 
                                color: '#ff6b35',
                                padding: '0 4px',
                                height: 'auto'
            ...
            {/* 没有更多数据提示 */}
            {!loadingMore && displayedVideos.length >= foodVideos.length && foodVideos.length > 0 && (
              <div className="no-more-container">
                <span>已显示全部 {foodVideos.length} 个美食视频</span>
              </div>
            )}
          </div>
        </LoadingState>
